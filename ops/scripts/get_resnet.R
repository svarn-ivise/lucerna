...
price_key <- qry("select * from price_key")
...
resnet.data <- read.csv(text=content(res, "text")) %>% 
  ...
  setNames(c("Travel.Date","Purchase.Date", "Service","booking_per_day","Experiment")) %>%
  ...
  summarise(booking_per_day = sum(booking_per_day), Price = round(mean(Price),4) ) %>% as.data.frame() %>%
  mutate(time = substr(Travel.Date,12,19), Travel.Date = as.Date(Travel.Date))

z <- qry("select * from intercity_new") %>% #filter(as.Date(Purchase.Date) < min(as.Date(resnet.data$Purchase.Date))) %>%
...
for(x in 1:nrow(resnet.data)){
  ...
  qty <- resnet.data$booking_per_day[x]
  ...
  seats <- sum(qry(paste0("
       ...
       where `Travel.Date` ='", travel.date,"' 
       and Service = '",service,"'"))$booking_per_day) + qty
